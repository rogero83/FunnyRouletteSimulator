...
{
    public class AlternateColorStrategy : IGameStrategy
    {
        public string Name => "Alternate color";
        ...
        public AlternateColorStrategy(decimal unit = 1m)
        {
            _unit = unit;
        ...
        public IEnumerable<Bet> NextBets(IReadOnlyList<Pocket> history, decimal currentBalance)
        {
            ...
            var numbers = _isRedTurn
                ? new HashSet<int>(Enumerable.Range(1, 36).Where(n => Wheel.IsRed(n)))
                : new HashSet<int>(Enumerable.Range(1, 36).Where(n => !Wheel.IsRed(n)));
            _lastBet = new Bet(betAmount, BetType.RedBlack, numbers);
